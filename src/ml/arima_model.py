...
logger = logging.getLogger("agentic.ml.arima")
...
def _directional_confidence(current_price: float, predicted_price: float) -> float:
    ...
    """
    if current_price <= 0:
        # A non-positive price is bad data, not a neutral signal. 0.5 here would be indistinguishable
        # from a genuine "no direction" reading and would be averaged into the metrics as one.
        raise ValueError(f"current_price must be positive to compute a direction, got {current_price}")
...
class ARIMAModel:
    ...
    def train(self, series: pd.Series | np.ndarray) -> dict:
        ...
        except Exception as e:  # noqa: BLE001 — statsmodels raises widely on non-convergence; the
            # failure is REPORTED in the return value rather than raised, because a sweep must be
            # able to record "this configuration did not fit" and carry on to the next one.
    ...
    def predict(self, series: pd.Series | np.ndarray, steps: int = 5) -> float:
        ...
        Raises:
            UntrainedModelError: statsmodels missing, too little history, or the fit did not
                converge. All three used to return 0.5 — a real, defensible probability, and
                therefore the worst possible placeholder: the validator averages whatever it gets,
                so a model that never produced a forecast scored near chance and read as merely
                weak. src/ml/validation.py counts these instead.
        """
        try:
            from statsmodels.tsa.arima.model import ARIMA
        except ImportError as exc:
            raise UntrainedModelError(
                "statsmodels is not installed, so ARIMA cannot forecast. The Lab runs as its own "
                "service precisely so this dependency need not exist in the main image."
            ) from exc
        ...
        if len(close_array) < 30:
            raise UntrainedModelError(
                f"ARIMA needs at least 30 observations and was given {len(close_array)}. A "
                f"forecast from too little history is not a weak forecast, it is no forecast."
            )
        ...
        try:
            ...
            raise UntrainedModelError(f"ARIMA did not converge: {exc}") from exc
    ...
    def load(self, path: str | Path) -> None:
        ...
        try:
            ...
            # which predict() then reports as UntrainedModelError rather than a placeholder forecast
