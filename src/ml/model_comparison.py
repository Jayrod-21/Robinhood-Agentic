...
logger = logging.getLogger("agentic.ml.model_comparison")
    ...
    def _ensemble_from_metrics(self) -> dict:
        ...
        for metric in ["accuracy", "sharpe_ratio", "win_rate", "max_drawdown", "f1"]:
            ...
            comparison[metric] = {
                ...
                # strict: models and values are built from the same loop, so a length mismatch is a
                # bug rather than something to silently truncate.
                "per_model": dict(zip(models, values, strict=True)),
            }
