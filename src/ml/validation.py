...
logger = logging.getLogger("agentic.ml.validation")
...
class ValidationResult:
    ...
    # How many predictions the model could not produce. EXCLUDED from `metrics` rather than
    # substituted, so a run where most predictions failed cannot present itself as a mediocre
    # model — the two look identical once a placeholder has been averaged in.
...
def calculate_metrics(predictions: np.ndarray, actuals: np.ndarray) -> dict:
    ...
    return {
        # True: these were computed from real predictions. Its absence or False means the numbers
        # beside it are placeholders — see _zeroed_metrics.
        "measured": True,
...
def _zeroed_metrics() -> dict:
    """A metrics dict for the case where nothing could be measured.

    `measured: False` is the important field. Every number below is a placeholder, and an accuracy
    of 0.0 reads as "always wrong" when the truth is "never ran" — a leaderboard sorting on accuracy
    would rank a model that never produced a prediction alongside one that produced only bad ones.
    Anything consuming these must check `measured` before treating them as results.
    """
    return {
        "measured": False,
        "accuracy": 0.0,
    ...
    def run_walk_forward(
        ...
        while current_test_start < n_samples:
            ...
            for i in range(current_test_start, current_test_end):
                ...
                if pred is None:
                    # Dropped in PAIRS: keeping the actual without its prediction would shift every
                    # later comparison by one and silently score the model against the wrong labels.
                    step_failed += 1
                    continue
            ...
            if step_failed:
                logger.warning(
                    "step %d: %d of %d predictions failed and are EXCLUDED from the metrics",
                    step_count, step_failed, current_test_end - current_test_start,
                )
        ...
        if total_failed:
            logger.warning(
                "%s: %d prediction(s) failed across the run and are excluded from every metric "
                "below. Read the metrics as describing the %d that succeeded.",
                model_name, total_failed, len(all_predictions),
            )
    ...
    def cross_validate(
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(splits):
            ...
            for i in range(len(X_test)):
                ...
                kept_actuals.append(y_test[i])

            if fold_failed:
                logger.warning(
                    "fold %d: %d of %d predictions failed and are EXCLUDED from the metrics",
                    fold_idx, fold_failed, len(X_test),
                )
            if not predictions:
                # Every prediction failed. Scoring an empty set would produce zeroed metrics that
                # look like a terrible model rather than an absent one.
                logger.error("fold %d: every prediction failed; the fold is not scored", fold_idx)
                continue

            metrics = calculate_metrics(np.array(predictions), np.array(kept_actuals))
            metrics["n_failed_predictions"] = fold_failed
...
def _safe_train(model, X_train, y_train, X_val=None, y_val=None) -> dict:
    ...
    try:
        ...
        except Exception as e:  # noqa: BLE001 — any training failure is reported, never raised
            logger.warning(f"Training failed: {e}")
            return {}
    except Exception as e:  # noqa: BLE001 — see above; a failed fit must not kill the sweep
        logger.warning(f"Training failed: {e}")
        return {}


def _safe_predict(model, features) -> float | None:
    """Predict, or None when the model cannot.

    CHANGED FROM THE SOURCE REPO, and this is the one substantive change in this file.

    It used to return 0.5 on any failure. 0.5 is a real, defensible probability — maximum
    uncertainty — which is exactly what makes substituting it dangerous here: the value flows
    straight into calculate_metrics at both call sites, so a model that failed EVERY prediction
    scores an accuracy near 50% and reads as "no better than chance". The truth in that case is
    "this model never ran", and those two findings have nothing in common except the number.

    A Testing Lab exists to tell a weak model from a broken one. Returning None makes the failure
    countable instead of averageable; the callers exclude these from the metrics and report how many
    there were, so a run that mostly failed cannot present itself as a mediocre one.
    """
    try:
        return float(model.predict(features))
    except Exception as e:  # noqa: BLE001 — a model's failure is data, not this function's problem
        # DEBUG, not WARNING: a broken model fails on every sample, and 200 identical warnings bury
        # the per-step aggregate that says how many and out of how many. The callers warn once.
