...
logger = logging.getLogger("agentic.ml.elastic_net")
    ...
    def predict(self, features: dict | np.ndarray) -> float:
        ...
        if self._model is None:
            raise UntrainedModelError(
                f"{type(self).__name__}.predict() called before train(). Returning a "
                f"placeholder here would be averaged into the metrics as if it were a real "
                f"forecast."
            )
